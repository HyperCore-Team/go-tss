...
func Test_Threshold_ECDSA_KeySign(t *testing.T) {
	...
	for i := 0; i < partyNum; i++ {
		...
		go func(idx int) {
			...
			keysignReq = keysign.NewRequest(poolPubKey, []string{base64.StdEncoding.EncodeToString([]byte("helloworld")), base64.StdEncoding.EncodeToString([]byte("helloworld2"))}, 10, localPubKeys, "0.13.0", "ecdsa")
	...
	wg.Wait()
	for i := 0; i < partyNum; i++ {
		if len(keysignResult[i].Blame.BlameNodes) > 0 {
			wg.Add(1)
			blameList := keysignResult[i].Blame.BlameNodes
			blamePubKeys := make([]string, 0)
			for _, peerID := range blameList {
				blamePubKeys = append(blamePubKeys, peerID.Pubkey)
			}
			var wrongIndices []int
			var localPubKeys []string
			for j := 0; j < partyNum; j++ {
				if contains(blamePubKeys, testPubKeys[j]) {
					wrongIndices = append(wrongIndices, j)
				} else {
					localPubKeys = append(localPubKeys, testPubKeys[j])
				}
			}
			go func(idx int) {
				defer wg.Done()
