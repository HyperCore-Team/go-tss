...
func Test_KeySign_With_Threshold(t *testing.T) {
	...
	for i := 0; i < partyNum; i++ {
		...
		go func(idx int) {
			...
			if idx == 2 {
				...
				keysignReq = keysign.NewRequest(poolPubKey, []string{base64.StdEncoding.EncodeToString([]byte("helloworld")), base64.StdEncoding.EncodeToString([]byte("helloworld3"))}, 10, localPubKeys, "0.13.0", "ecdsa")
			}
