...
func (s *FourNodeTestSuite) getEddsaServer(index int, conf common.TssConfig, bootstrap string, testPriKeyArr []string, whiteList map[string]bool) *tss.TssServer {
	...
	instance, err := tss.NewTss(peerIDs, s.ports[index], priKey, "Asgard", baseHome, conf, nil, "", messages.EDDSAKEYGEN, whiteList)
	return instance
}

func Test_EDDSA_Keygen(t *testing.T) {
