...
func Test_Complex_EDDSA_Resharing(t *testing.T) {
	...
	for i := 0; i < config.partyNum+config.newPartyNum; i++ {
		req := keyRegroup.NewRequest(pubKey, config.pubKeys[1:], config.pubKeys[:4], config.blockHeight, config.version, "eddsa")
		wg.Add(1)
		go func(idx int) {
			defer wg.Done()
			if idx == 0 {
