...
func Test_EDDSA_Resharing(t *testing.T) {
	...
	config := &Config{
		...
		bootstrapPeer: "/ip4/127.0.0.1/tcp/18667/p2p/12D3KooWSAumwg2rxzjsgv7LuWM4u3HqfLcnekg2NHc5TsNj5hgC",
	...
	for i := 0; i < config.partyNum+config.newPartyNum; i++ {
		...
		go func(idx int) {
			defer wg.Done()
			if idx == 0 {
				config.servers[idx] = config.getTssServer(idx, conf, "", messages.EDDSAKEYREGROUP, whiteList)
			} else {
				config.servers[idx] = config.getTssServer(idx, conf, config.bootstrapPeer, messages.EDDSAKEYREGROUP, whiteList)
			}
		}(i)
	...
	for i := 0; i < config.partyNum; i++ {
		...
		go func(idx int) {
			...
			req = keygen.NewRequest(localPubKeys, config.blockHeight, config.version, "eddsa")
	...
	for i := 0; i < config.partyNum+config.newPartyNum; i++ {
		req := keyRegroup.NewRequest(pubKey, config.pubKeys[config.oldPartyStartOffset:], config.pubKeys[:config.partyNum], config.blockHeight, config.version, "eddsa")
		wg.Add(1)
		go func(idx int) {
			defer wg.Done()
			if idx == 0 {
